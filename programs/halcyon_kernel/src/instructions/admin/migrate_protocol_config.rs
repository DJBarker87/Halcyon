...
pub fn handler(ctx: Context<MigrateProtocolConfig>) -> Result<()> {
    ...
    if current_len >= target_len {
        ...
        require_keys_eq!(cfg.admin, ctx.accounts.admin.key(), HalcyonError::AdminMismatch);
        return Ok(());
    }

    let legacy = ProtocolConfigV4::try_from_slice(&data[8..])
    ...
    protocol_config.to_account_info().realloc(target_len, true)?;
