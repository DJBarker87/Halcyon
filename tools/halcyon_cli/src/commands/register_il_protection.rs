...
use halcyon_client_sdk::{
    decode::fetch_anchor_account_opt, kernel::register_il_protection_ix, pda, tx::send_instructions,
};
...
pub struct Args {
    ...
    #[arg(long, default_value_t = 6_000_000_000)]
...
pub async fn run(ctx: &CliContext, args: Args) -> Result<()> {
    ...
    .is_some()
    {
        println!("IL Protection already registered at {product_registry_entry}");
        return Ok(());
    }

    let ix = register_il_protection_ix(
        &admin.pubkey(),
    ...
    println!(
        "registered IL Protection: sig={sig} registry={} sigma={}",
        product_registry_entry,
        pda::vault_sigma(&halcyon_il_protection::ID).0,
    );
    Ok(())
}
