...
use halcyon_client_sdk::{
    decode::fetch_anchor_account_opt, kernel::register_flagship_autocall_ix, pda,
...
pub async fn run(ctx: &CliContext, args: Args) -> Result<()> {
    ...
    .is_some()
    {
        println!("Flagship Autocall already registered at {product_registry_entry}");
        return Ok(());
    }

    let ix = register_flagship_autocall_ix(
        &admin.pubkey(),
    ...
    println!(
        "registered Flagship Autocall: sig={sig} registry={} sigma={}",
        product_registry_entry,
