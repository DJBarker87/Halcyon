...
- `RegimeSignal` (one per product that uses regime switching — currently IL Protection) — current fvol value, current regime enum (calm/stress), sigma multiplier (1.30 calm / 2.00 stress), sigma floor (40% annualised), last update timestamp. Written by the regime keeper, not permissionless (fvol requires off-chain historical computation).
...
**Regime keeper.** Daily compute of fvol from historical price history (off-chain). Writes `RegimeSignal` account with the current regime (calm/stress), sigma multiplier (1.30 / 2.00), sigma floor (40% annualised). Runs after the Pyth feed's daily-close window. Keeper-authority-gated write; rejected if the previous regime signal is less than 18 hours old.
