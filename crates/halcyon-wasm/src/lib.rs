...
use halcyon_flagship_quote::k12_correction::k12_correction_lookup;
use halcyon_flagship_quote::worst_of_c1_fast::spy_qqq_iwm_c1_config;
use halcyon_flagship_quote::worst_of_c1_filter::quote_c1_filter;
...
use halcyon_sol_autocall_quote::autocall_v2_e11::{
    live_quote_uses_e11, solve_fair_coupon_e11_cached,
};
...
pub extern "C" fn sm_norm_cdf(x: f64) -> f64 {
    let Some(xf) = to_fp12_i(x) else {
        return f64::NAN;
    };
    match norm_cdf_poly(xf) {
        Ok(c) => c as f64 / SCALE_12_F,
        Err(_) => f64::NAN,
    }
}
...
pub extern "C" fn sm_erf(x: f64) -> f64 {
    ...
    let Some(xf) = to_fp12_i(scaled) else {
        return f64::NAN;
    };
    match norm_cdf_poly(xf) {
        Ok(c) => 2.0 * (c as f64 / SCALE_12_F) - 1.0,
        Err(_) => f64::NAN,
    }
}
...
pub extern "C" fn sm_compute_il(w: f64, x: f64) -> f64 {
    let (Some(wf), Some(xf)) = (to_fp12_u(w), to_fp12_u(x)) else {
        return f64::NAN;
    };
    match compute_il(wf, xf) {
        Ok(il) => il as f64 / SCALE_12_F,
        Err(_) => f64::NAN,
    }
}

#[no_mangle]
pub extern "C" fn sm_barrier_hit_prob(
    spot: f64,
    barrier: f64,
    sigma: f64,
    t: f64,
...
pub extern "C" fn sm_barrier_option(
    ...
    let bt = match barrier_kind {
        0 => BarrierType::DownAndOut,
        1 => BarrierType::DownAndIn,
        2 => BarrierType::UpAndOut,
        3 => BarrierType::UpAndIn,
        _ => return f64::NAN,
    };
...
pub extern "C" fn sm_sol_fair_coupon(sigma_ann: f64) -> f64 {
    if !sigma_ann.is_finite() || sigma_ann <= 0.0 {
        return f64::NAN;
    }
    let Some(sigma6) = to_fp6_i(sigma_ann) else {
        return f64::NAN;
    };
    ...
    if live_quote_uses_e11(sigma_ann, &contract) {
        if let Ok(r) =
            solve_fair_coupon_e11_cached(sigma6, SOL_ALPHA_6, SOL_BETA_6, SOL_STEP_DAYS, &contract)
        {
    ...
    use halcyon_sol_autocall_quote::autocall_v2::NigParams6;
    let nig =
        match NigParams6::from_vol_with_step_days(sigma6, SOL_ALPHA_6, SOL_BETA_6, SOL_STEP_DAYS) {
            Ok(p) => p,
...
pub extern "C" fn sm_worst_of_k12_coupon_bps(sigma_ann: f64) -> f64 {
    ...
    let Some(sigma_s6) = to_fp6_i(sigma_ann) else {
        return f64::NAN;
    };
